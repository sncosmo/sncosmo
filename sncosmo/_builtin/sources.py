...
def load_snana_ccsn_model(pkg_data_name, name=None, version=None):
    fname = get_pkg_data_filename(pkg_data_name)
    phase, wave, flux = io.read_griddata_ascii(fname)
    # The SNANA .SED files have flux scaled to D = 10 pc
    #  to convert that into erg / s / cm^2 / Angstrom
    # 10 pc = 3.08567758e19 cm
    # (10 pc)^2 = 9.521406127714657e+38 cm^2
    return TimeSeriesSource(phase, wave, flux, name=name, version=version)
...
for modelname,sedfile,type in [
    ...
    registry.register_loader(Source, modelname, load_snana_ccsn_model,
                             args=['https://www.dropbox.com/sh/vniwfspu09hxcea/AAC6HRDknn5zMwq8Qn9tdNTca?dl=0/'+sedfile],
                             version='1.0',
                             meta=meta )
...
for mod in ['z15B','z15D','z15G','z25B','z25D','z25G','z40B','z40G'] :
    registry.register_loader(Source,mod,load_snana_ccsn_model,
                             args=['https://www.dropbox.com/sh/vt1hky5mmiouy1y/AACcUzND4kJmWi7StvQ9mG5aa?dl=0/popIII-%s.sed.restframe10pc.dat'%mod],
                             version='1.0',
